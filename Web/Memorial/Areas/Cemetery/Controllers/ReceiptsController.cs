...
namespace Memorial.Areas.Cemetery.Controllers
{
    public class ReceiptsController : Controller
    {
        ...
        public ActionResult Index(string IV)
        {
            ...
            var transaction = _transaction.GetByAF(invoice.CemeteryTransactionAF);
            var receipts = _receipt.GetByIV(IV).OrderByDescending(r => r.CreatedUtcTime);
            var viewModel = new OrderReceiptsViewModel()
            {
                AF = invoice.CemeteryTransactionAF,
                AFTotalAmount = _transaction.GetTotalAmount(transaction),
                AFTotalAmountPaid = _receipt.GetTotalIssuedReceiptAmountByAF(invoice.CemeteryTransactionAF),
        ...
        public ActionResult Info(string RE, bool exportToPDF = false)
        {
            ...
            var transaction = _transaction.GetByAF(receipt.CemeteryTransactionAF);
        ...
        public ActionResult Save(NewOrderReceiptFormViewModel viewModel)
        {
            ...
            receipt.CemeteryTransactionAF = viewModel.AF;
            ...
            if (viewModel.ReceiptDto.RE == null)
            {
                ...
                if (_receipt.Add(transaction.CemeteryItemId, receipt))
                {
