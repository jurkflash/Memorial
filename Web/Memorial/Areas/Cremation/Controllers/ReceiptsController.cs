...
namespace Memorial.Areas.Cremation.Controllers
{
    public class ReceiptsController : Controller
    {
        ...
        public ActionResult Index(string IV)
        {
            ...
            {
                AF = invoice.CremationTransactionAF,
                AFTotalAmount = _transaction.GetTotalAmount(transaction),
                AFTotalAmountPaid = _receipt.GetTotalIssuedReceiptAmountByAF(invoice.CremationTransactionAF),
        ...
        public ActionResult Info(string RE, bool exportToPDF = false)
        {
            ...
            var transaction = _transaction.GetByAF(receipt.CremationTransactionAF);
        ...
        public ActionResult Save(NewOrderReceiptFormViewModel viewModel)
        {
            ...
            receipt.CremationTransactionAF = viewModel.AF;

            if (viewModel.ReceiptDto.RE == null)
            {
                receipt.InvoiceIV = viewModel.InvoiceDto.IV;

                if (_receipt.Add(transaction.CremationItemId, receipt))
                {
