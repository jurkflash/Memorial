...
namespace Memorial.Persistence.Repositories
{
    public class CemeteryTransactionRepository : Repository<CemeteryTransaction>, ICemeteryTransactionRepository
    {
        ...
        public CemeteryTransaction GetByAF(string AF)
        {
        ...
        {
            return MemorialContext.CemeteryTransactions.Where(pt => pt.ApplicantId == id).Any();
        }

        public bool GetExistsByDeceased(int id)
        {
            return MemorialContext.CemeteryTransactions.Where(pt => pt.Deceased1Id == id || pt.Deceased2Id == id || pt.Deceased3Id == id).Any();
        }
        ...
        public IEnumerable<CemeteryTransaction> GetRecent(int? number, byte? siteId, int? applicantId)
        {
            ...
            if (siteId != null)
                result = result.Where(t => t.CemeteryItem.Plot.CemeteryArea.SiteId == siteId);
